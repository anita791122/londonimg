...
>
> **絕對不可自創版面、不可改任何 style、不可加 font-family。**（唯一可改的非 ✏️ 文字：收尾 CTA 按鈕字樣「回上方選你的機型・立即加入購物車」——選購維度不是機型時把「機型」換成對應詞（錶帶→錶徑、線材→長度、充電頭→瓦數）；完全沒有選購情境才整句改「回上方・立即加入購物車」。只改字、不動樣式。）
  ...
  <div style="padding:24px 0 4px">
    <div style="position:relative;overflow:hidden;background:#0d0f0c">
      ...
      <div style="position:absolute;left:0;right:0;top:7%;text-align:left;color:#fff;text-shadow:0 2px 14px rgba(0,0,0,.4);padding:0 24px;z-index:2">
        <div style="font-size:clamp(13px,2.4vw,16px);font-weight:600;letter-spacing:3px;margin:0 0 10px;color:#fe5226">✏️【英文小標，例：EVERYDAY CARRY】</div>
...
## 03 影音 — 版本 A：直式 9:16（像 Shorts）

```html
<!-- ✏️ 改這裡：影片換 src 網址、文字填【】。長長的 style="…" 不要動。 -->
<!-- 影音（版本 A：直式 9:16）｜橫滑影片列 -->
<div style="max-width:1080px;margin:0 auto;padding:0 20px">
  <div style="padding:clamp(16px,4vw,28px) 0;text-align:center">
...
<div style="position:relative;width:100vw;left:50%;margin-left:-50vw;overflow-x:clip">
  <div style="padding:clamp(16px,4vw,28px) 0">
    ...
    <div style="display:flex;flex-wrap:nowrap;overflow-x:auto;padding:18px 24px 14px;padding-left:max(24px,calc((100% - 1080px)/2));scroll-padding-left:max(24px,calc((100% - 1080px)/2));scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch">
  ...
  <div style="padding:clamp(16px,4vw,28px) 0">
    ...
    <div style="display:flex;flex-wrap:nowrap;overflow-x:auto;padding:18px 24px 14px;padding-left:max(24px,calc((100% - 1080px)/2));scroll-padding-left:max(24px,calc((100% - 1080px)/2));scroll-snap-type:x mandatory;-webkit-overflow-scrolling:touch">
      ...
      <div style="flex:0 0 auto;width:300px;scroll-snap-align:start;margin-right:14px">
        ...
        <div style="font-size:clamp(16px,2vw,18px);color:#6E6E73;line-height:1.5;margin:10px 2px 0">✏️【圖下說明，例：多角度細節實拍】</div>
      </div>
      ...
      <div style="flex:0 0 auto;width:300px;scroll-snap-align:start;margin-right:14px">
        ...
        <div style="font-size:clamp(16px,2vw,18px);color:#6E6E73;line-height:1.5;margin:10px 2px 0">✏️【圖下說明，例：側面防摔結構】</div>
      </div>
...
## 07 多圖橫移（方形滑塊） — 版本 C：圖下有說明文字（每張圖下面帶一行敘述）
...
> 此塊語法完整可用（教學頁已開放）。IG 帳號固定 `@london_apple`；只放真實照片。
