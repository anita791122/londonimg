...
settings_page = f'''
  ...
  <div class="card">
    ...
    <p class="tipbox">{IC["bulb"]}<span><b>原則：頁面簡短介紹要「短」</b>——一句賣點＋優惠重點就好，讓使用者一眼看到優惠。<b>詳細的賣點、規格、使用情境全部放 STEP 2「商品敘述（語法）」</b>，不要塞進簡短介紹。需要時請 Claude Skill「幫我寫頁面簡短介紹」即可產出短版。</span></p>
  </div>
  <div class="card warn">
...
<div class="layout">
  <aside class="sidebar">
    ...
    <nav class="nav">
      ...
      <a data-t="p1" onclick="show('p1')">{IC["settings"]}<span>商品頁設定</span><span class="step">03</span></a>
      <a data-t="p3" onclick="show('p3')">{IC["image"]}<span>圖片取得方式</span><span class="step">04</span></a>
    </nav>
