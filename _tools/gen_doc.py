...
meta = [
    ("影音實測", "兩支 YouTube 影片並排。<b>換影片</b>：到 YouTube 影片 → 按「分享」→「嵌入」→ 複製整段 <code>&lt;iframe&gt;</code> 程式碼，貼到 ✏️ 那格（取代整個 ✏️【…】）；<b>換文字</b>：填影片標題與說明。<br>📦 影片大小由 YouTube 嵌入碼決定；想要<b>直式</b>（像 Shorts）就把嵌入碼裡的 <code>width</code>／<code>height</code> 改成例如 <code>335</code>／<code>596</code>。<b>沒有影片就把整個「影音實測」區塊刪掉</b>。"),
    ...
    ("特色區①（標題＋三張方圖）", "一個賣點＝一個特色區：英文小標＋中文大標＋一句說明＋三張正方圖。換圖改三個 <code>&lt;img&gt;</code>、換字填標題與說明。"),
    ("特色區②（標題＋兩張圖）", "一個賣點：英文小標＋大標＋說明＋兩張圖。換圖換字方式同上。"),
    ("特色區③（標題＋兩張圖）", "一個賣點：英文小標＋大標＋說明＋兩張圖。換圖換字方式同上。"),
    ...
    ("特色區④（兩張含說明圖）", "一個賣點（圖文配對版）：英文小標＋大標＋說明＋兩張各帶圖說的圖。換圖改 <code>&lt;img&gt;</code>、圖說填 <code>&lt;figcaption&gt;</code>。"),
    ("一個支架，多種玩法（四個情境）", "四個使用情境，桌機左右交錯、手機自動變上下。每段填情境標題與說明、換圖。要加情境就複製其中一段再改。"),
    ("功能特色（背景圖疊三行字）", "背景圖疊三行特色文字。換背景圖改 <code>&lt;img&gt;</code>，填小標、大標與三行特色。"),
    ("適用機型（可見表格）", "標題＋一張相容機型表格（對 SEO/GEO 有幫助、使用者也清楚）。把表格列改成你的商品相容機型，✅／❌ 依實際填，<b>可自行增減列</b>（複製一整列 <code>&lt;tr&gt;…&lt;/tr&gt;</code> 再改）。"),
    ("規格", "標題＋規格清單。把每列「項目：內容」改成你的商品規格（材質／尺寸／容量…），<b>可自行增減列</b>（複製一整列 <code>&lt;div&gt;…&lt;/div&gt;</code> 再改）。"),
...
edits = [
    ...
    [("一般款？雙向吸","【請填入對照大標，例：一般款？升級款】"),
     ...
     ("● 支援","● 【填結果】"),
     ("● 無","● 【填結果】"),
     (">Compare</div>",">【英文小標】</div>")],
    [("一般款 vs 3.0 超進化","【請填入區塊標題，例：一般款 vs 升級款】"),
     (">Two Versions</div>",">【英文小標】</div>"),
     (">一般款</span>",">【標籤1】</span>"),
    ...
    # 適用機型
    [("Compatibility","【英文小標】"),
     ("iPhone 16／16 Plus／16 Pro／16 Pro Max","【請填入相容機型，例：iPhone 16 全系列】"),
     ("iPhone 15／14／13／12 全系列","【請填入相容機型】"),
     (">各系列 mini</td>",">【請填入不相容機型，例：各系列 mini】</td>")],
    # 規格
    [(">SPECS</div>",">【英文小標】</div>"),
     (">材質</span>",">【規格項目，例：材質】</span>"),
     ("TPU＋PC 複合","【請填入內容】"),
     (">支架</span>",">【規格項目】</span>"),
     ("360° 無段金屬轉軸","【請填入內容】"),
     (">充電</span>",">【規格項目】</span>"),
...
settings_page = f'''
  <div class="hero">
    <span class="hero-eyebrow">STEP 4</span>
    <h1 class="hero-title">商品頁設定</h1>
    <p class="hero-sub">後台位置：<b>商品 → 編輯該商品 → 網路搜尋最佳化（SEO）</b>。把下面四欄分別複製貼上即可。</p>
  </div>
  <div class="howto">
    <div class="howto-h">{IC["skill"]}要做「其他商品」嗎？用你裝好的 Claude Skill 產生</div>
    下面四欄是 <b class="key">B67 的範例值</b>，<b>不是每個商品都一樣</b>。換別的商品時，用 <b>STEP 1 裝好的 Claude Skill</b>，對 Claude 說「<i>幫 [商品名] 寫商品頁文案</i>」，它會產出<b>該商品專屬</b>的標題／簡介／關鍵字／描述性URL，複製回來貼即可。<br>
  ...
  <div class="card">
    ...
    <p class="tipbox">{IC["bulb"]}<span><b>原則：頁面簡短介紹要「短」</b>——一句賣點＋優惠重點就好，讓使用者一眼看到優惠。<b>詳細的賣點、規格、使用情境全部放 STEP 3「商品敘述（語法）」</b>，不要塞進簡短介紹。需要時請 Claude Skill「幫我寫頁面簡短介紹」即可產出短版。</span></p>
  </div>
  <div class="card warn">
...
images_page = f'''
  <div class="hero">
    <span class="hero-eyebrow">STEP 5</span>
  ...
  <div class="howto">
    <div class="howto-h">{IC["skill"]}Claude Skill 不會幫你上傳圖片</div>
    Claude Skill 產出的是<b>文字與語法</b>；<b>圖片仍要你自己上傳到 SHOPLINE 取得網址</b>，再把網址填進語法的 <code>src</code>。這頁就是教你怎麼拿到那個網址。
  </div>
  <div class="card">
...
skill_page = f'''
  <div class="hero">
    <span class="hero-eyebrow">STEP 1 · 先裝這個</span>
    <h1 class="hero-title">Claude Skill</h1>
    <p class="hero-sub">第一步：先安裝這個 skill。之後寫任何商品的文案都靠它自動產出。下載 → 安裝 → 使用三步驟。</p>
  </div>

  <div class="card">
    <h3 class="card-h">{IC["download"]}下載區</h3>
    <p>檔案：<code>londonimg-product.zip</code>　·　用途：輸入商品資訊，自動產出 SHOPLINE 商品頁的 <b>SEO 後台四欄</b>（標題／簡介／關鍵字／描述性URL）＋<b>商品描述文案</b>（定義句／段落標題／常見問答）。</p>
    <p><a class="dl-btn" href="londonimg-product.zip" download>{IC["download"]}<span>下載 Skill（.zip）</span></a></p>
  ...
  <div class="card">
    ...
    <ol class="steps">
      ...
      <li>按 <b>新增 / 上傳 Skill</b>，選剛下載的 <code>londonimg-product.zip</code> → 開啟啟用。</li>
  ...
  <div class="card">
    ...
    <ol class="steps">
      ...
      <li>對著貼：商品名稱／商品摘要 → <b>STEP 2 商品資訊</b>；商品描述 → <b>STEP 3 商品頁語法</b>（找同名區塊填 ✏️【…】）；SEO 四欄 → <b>STEP 4 商品頁設定</b>。</li>
...
product_info_page = f'''
  <div class="hero">
    ...
    <h1 class="hero-title">商品資訊</h1>
    <p class="hero-sub">後台位置：<b>商品 → 編輯該商品 → 商品資訊</b>。這裡設「<b>商品名稱</b>」與「<b>商品摘要</b>」（價格上方、使用者看得到的簡短介紹／促銷）。</p>
  </div>

  <div class="card">
    <h3 class="card-h">{IC["tag"]}商品名稱</h3>
    <p>顯示在商品頁最上方的名稱（也是頁面大標）。用 <b>STEP 1 Claude Skill</b> 產的「標題」即可；若含內部代號（如測試前綴）記得拿掉。</p>
    {field("商品名稱（繁體中文）", "YU iPhone 360°無段支架手機殼｜MagSafe 防摔殼（12-16 系列）【B67】", "顧客實際看到的商品名。")}
    {field("商品名稱（英文）", "YU iPhone 360° Ring Stand Case｜MagSafe (12-16)", "顧客把語言切成英文才會看到；沒有可留空。")}
    <p class="tipbox">{IC["info"]}<span>「商品名稱」是<b>頁面上顯示</b>的標題；STEP 4「網路搜尋最佳化」的標題是<b>給 Google</b> 的，兩者可以不同（但通常相近）。</span></p>
  </div>

  <div class="card">
    <h3 class="card-h">{IC["info"]}商品摘要（價格上方的簡短介紹／促銷）</h3>
    <p>就是商品頁價格上方那塊（🎁 驚喜賣場…／🚚 湊免運…）。<b>要短、以優惠為主</b>；詳細賣點放 STEP 3 商品描述。下面是促銷框範本，可直接複製貼進「商品摘要」（切 HTML 原始碼模式）。</p>
    <div class="code">
      <div class="code-bar"><span class="dots"><i></i><i></i><i></i></span><span class="fname">商品摘要 HTML</span><button class="copy-btn" onclick="copyCode(this)">{IC["copy"]}<span class="lbl">複製</span></button></div>
      <textarea readonly spellcheck="false">&lt;div class="promobox"&gt;
  &lt;div class="promo-line"&gt;🎁 驚喜賣場！&lt;b&gt;任選 2 件享 88 折&lt;/b&gt;（不含加購品）&lt;/div&gt;
  &lt;div class="promo-line"&gt;🚚 湊免運嗎？&lt;b&gt;&lt;a href="https://www.londonimg.tw/pages/add" target="_blank"&gt;加價購&lt;/a&gt;省更多｜超值好物一起帶！&lt;/b&gt;&lt;/div&gt;
&lt;/div&gt;</textarea>
    </div>
    <p class="tipbox">{IC["bulb"]}<span>這段用的是店家既有的促銷框樣式（<code>promobox</code>），全店共用。<b>折扣與連結依當前檔期</b>——要改活動就改 <code>&lt;b&gt;…&lt;/b&gt;</code> 裡的文字；多數商品這段直接照貼即可。也可請 Claude Skill「幫我寫商品摘要」產出短版。</span></p>
  </div>'''
...
<div class="layout">
  <aside class="sidebar">
    ...
    <nav class="nav">
      <a data-t="p4" class="active" onclick="show('p4')">{IC["skill"]}<span>Claude Skill</span><span class="step">01</span></a>
      <a data-t="p5" onclick="show('p5')">{IC["tag"]}<span>商品資訊</span><span class="step">02</span></a>
      <a data-t="p2" onclick="show('p2')">{IC["code"]}<span>商品頁語法</span><span class="step">03</span></a>
      <a data-t="p1" onclick="show('p1')">{IC["settings"]}<span>商品頁設定</span><span class="step">04</span></a>
      <a data-t="p3" onclick="show('p3')">{IC["image"]}<span>圖片取得方式</span><span class="step">05</span></a>
    </nav>
    <div class="side-foot">標 <b style="color:var(--accent)">✏️【…】</b> 的地方都是填空處。<br>填好再貼進 SHOPLINE 商品描述。</div>
  </aside>

  <main class="content">
    <section id="p1" class="page">{settings_page}
    </section>
    ...
    <section id="p2" class="page">
      ...
      <div class="howto">
        ...
        操作：按「複製語法」→ 把【】都填好 → 到 SHOPLINE <b>商品 → 商品描述 → 右上角 &lt;/&gt; 切 HTML 原始碼</b> → 貼上 → 存檔。<br>
        <span class="key">想省事？</span>用 <b>STEP 1 的 Claude Skill</b> 對 Claude 說「幫 [商品] 寫商品頁文案，<b>也給我可貼的 HTML</b>」，它會直接把填空處填好給你；這頁則是手動填空＋對照範例用。
      </div>
      {blocks_joined}
    </section>
    ...
    <section id="p4" class="page active">{skill_page}
    </section>

    <section id="p5" class="page">{product_info_page}
    </section>
  </main>
</div>
